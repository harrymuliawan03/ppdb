...
<div class="form-group col-sm-6">
    ...
    {!! Form::select(
        'school_classes',
        ['' => 'Select Class'] + $classes->toArray(),
        old('school_classes', $studentGrade->school_class_id),
        [
            'class' => 'form-control',
            'id' => 'school_classes',
        ],
...
<div class="form-group col-sm-6">
    ...
    {!! Form::select('student_id', $students, old('student_id', $studentGrade->student_id), [
        'class' => 'form-control',
        'id' => 'student_id',
    ]) !!}
</div>
...
<div class="form-group col-sm-6">
    ...
    {!! Form::select('teacher_id', $teachers, null, ['class' => 'form-control', $isTeacher ? 'disabled' : '']) !!}
</div>
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            $.each(data, function(id, name) {
                                $('#student_id').append('<option value="' + id + '">' +
                                    name + '</option>');
                            });
                        }
                    });
                } else {
                    $('#student_id').empty(); // Clear student dropdown if no class is selected
                }
            });
            $('.dropify').dropify({
                messages: {
